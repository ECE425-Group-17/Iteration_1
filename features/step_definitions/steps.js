...
async function getAiMessageCount() {
  return page.$$eval('.chat-row.ai', elements => elements.length);
}

async function waitForNextAiMessage(previousCount) {
  await page.waitForFunction((count) => {
    const messages = document.querySelectorAll('.chat-row.ai');
    const lastMessage = messages[messages.length - 1];
    return messages.length > count && lastMessage && lastMessage.innerText.trim().length > 0;
  }, { timeout: 30000 }, previousCount);
...
When('I send the message {string}', async function (message) {
  const previousAiMessageCount = await getAiMessageCount();
  await page.click('#userInput', { clickCount: 3 });
  await page.keyboard.press('Backspace');
  await page.type('#userInput', message);
  await page.click('#sendBtn');
  this.lastAiMessage = await waitForNextAiMessage(previousAiMessageCount);
});
...
Given('I have generated a itinerary', async function () {
  ...
  const responseText = await waitForNextAiMessage(previousAiMessageCount);
  this.lastAiMessage = responseText;
  assert(responseText.includes('Travel Itinerary'));
});

Then('the response should be generated by the Llama model', async function () {
  const responseText = this.lastAiMessage || await waitForLastAiMessage();
