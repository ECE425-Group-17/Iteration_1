...
async function handleChat(userMessage, selectedModels = null) {
  try {
    const places = await searchGooglePlaces(userMessage);
...
Use ONLY the real Google Places results below.
...
`;
    ...
    const finalPrompt = isItineraryRequest ? itineraryPrompt : ragPrompt;
